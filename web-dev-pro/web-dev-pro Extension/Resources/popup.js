...
function renderSEO(result) {
    ...
    renderKeyValues("seo-output", {
        ...
        "Twitter card tags": result.twitterCount,
...
async function handleStorageAction(event) {
    ...
    const item = lastStoragePayload.items.find((entry) => entry.kind === kind && entry.key === key);
    if (!item) {
        return;
    }
...
async function runAction(action) {
    ...
    try {
        if (action === "a11y-filter-none") {
            ...
            await sendToActiveTab({ action: "a11y-color-filter", filter: "protanopia" });
        } else if (action === "a11y-filter-deuteranopia") {
            await sendToActiveTab({ action: "a11y-color-filter", filter: "deuteranopia" });
        } else if (action === "a11y-filter-tritanopia") {
            await sendToActiveTab({ action: "a11y-color-filter", filter: "tritanopia" });
        } else if (action === "seo") {
            ...
            await navigator.clipboard.writeText(JSON.stringify(lastStoragePayload, null, 2));
